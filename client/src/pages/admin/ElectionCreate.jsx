...
export default function ElectionCreate() {
  ...
  useEffect(() => {
    api.get('/api/templates').then(res => setTemplates(res.data.templates)).catch(() => {});
  }, []);

  // Auto-set auth method when election scope changes
  useEffect(() => {
    if (form.election_scope === 'general') {
      setForm(prev => ({ ...prev, auth_method: 'voter_code' }));
    }
  }, [form.election_scope]);
  ...
  const applyTemplate = (template) => {
    ...
    setForm(prev => ({
      ...
      event_type: config.event_type || prev.event_type,
      election_scope: config.election_scope || prev.election_scope,
      voting_type: config.voting_type || prev.voting_type,
      auth_method: config.auth_method || prev.auth_method,
      price_per_vote: config.price_per_vote || prev.price_per_vote,
    }));
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/api/elections', form);
      toast.success('Election created successfully!');
      navigate(`/admin/elections/${res.data.election.id}`);
    } catch (err) {
      toast.error(err.response?.data?.error?.message || 'Failed to create election');
    } finally {
      setLoading(false);
    }
  };

  const selectedEventType = EVENT_TYPE_OPTIONS.find(o => o.value === form.event_type);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-surface-900">Create Election</h1>
        <p className="text-surface-500 mt-1">Set up a new election with all the details</p>
      </div>
        ...
        {templates.length > 0 && (
          <div className="bg-primary-50 border border-primary-200 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-semibold text-primary-800">Start from a Template</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {templates.map(t => (
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-medium bg-white border border-primary-200 rounded-lg text-primary-700 hover:bg-primary-100 transition-colors"
                >
                  {t.name}
                </button>
              ))}
          ...
          <div className="px-6 py-4 border-b border-surface-100 bg-surface-50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-surface-400">Give your election a name and description</p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-surface-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-surface-100 bg-surface-50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-surface-800">Election Scope</h2>
                <p className="text-xs text-surface-400">Choose who can participate in this election</p>
              </div>
            </div>
          </div>
          <div className="p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => setField('election_scope', 'institution')}
                className={`flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all duration-200 ${
                  form.election_scope === 'institution'
                    ? 'border-primary-400 bg-primary-50 ring-2 ring-primary-500'
                    : 'border-surface-200 hover:border-surface-300 hover:bg-surface-50'
                }`}
              >
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  form.election_scope === 'institution' ? 'bg-primary-100 text-primary-600' : 'bg-surface-100 text-surface-400'
                }`}>
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ="round" stroke<path strokeLinecapLinejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5" />
                  </svg>
                </div>
                <div>
                  <span className={`text-sm font-semibold ${form.election_scope === 'institution' ? 'text-primary-700' : 'text-surface-700'}`}>
                    Institution / School
                  </span>
                  <p className="text-xs text-surface-400 mt-0.5">Only registered students with IDs can vote</p>
                </div>
              </button>
              <button
                type="button"
                onClick={() => setField('election_scope', 'general')}
                className={`flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-all duration-200 ${
                  form.election_scope === 'general'
                    ? 'border-primary-400 bg-primary-50 ring-2 ring-primary-500'
                    : 'border-surface-200 hover:border-surface-300 hover:bg-surface-50'
                }`}
              >
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  form.election_scope === 'general' ? 'bg-primary-100 text-primary-600' : 'bg-surface-100 text-surface-400'
                }`}>
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <div>
                  <span className={`text-sm font-semibold ${form.election_scope === 'general' ? 'text-primary-700' : 'text-surface-700'}`}>
                    General / Public
                  </span>
                  <p className="text-xs text-surface-400 mt-0.5">Anyone with a voting code can participate</p>
                </div>
              </button>
            </div>

            {/* Auth method tip based on scope */}
            <div className="mt-6 p-4 bg-surface-50 rounded-xl">
              ...
              {form.election_scope === 'institution' ? (
                <div className="text-sm text-surface-600">
                  <p>For institution elections, you can choose between:</p>
                  <ul className="mt-2 space-y-1 text-xs">
                    <li>- <span className="font-medium">Student ID + PIN:</span> Voters log in with their registered student ID and PIN</li>
                    <li>- <span className="font-medium">Voter Code:</span> Generate unique codes for each voter</li>
                  </ul>
                </div>
              ) : (
                <div className="text-sm text-surface-600">
                  <p>For public/general elections, only <span className="font-medium text-purple-700">Voter Code</span> authentication is available. Voters will receive a unique code to cast their vote.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-surface-700 mb-3">
                ...
                {form.election_scope === 'general' && (
                  <span className="text-xs font-normal text-surface-400 ml-2">(Only voter codes available for public elections)</span>
                )}
              </label>
              <div className={`grid grid-cols-1 gap-3 ${form.election_scope === 'general' ? 'sm:grid-cols-1' : 'sm:grid-cols-2'}`}>
                {/* Student ID + PIN - Only for Institution */}
                {form.election_scope !== 'general' && (
                  <button
                    type="button"
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-surface-400 mt-0.5">
                      {form.election_scope === 'general' 
                        ? 'Anyone with a code can vote (public election)'
                        : 'Generate and distribute unique voting codes to voters'}
                    </p>
                  </div>
                </button>
              </div>
          ...
          <div className="px-6 py-4 border-b border-surface-100 bg-surface-50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-surface-400">Set the start and end dates for voting</p>
              </div>
            </div>
          </div>
          <div className="p-6">
